...
def display_select_mode():
    ...
    回答モードのラジオボタンを表示
    """
    # サイドバーに「社内文書検索」と「社内問い合わせ」の選択肢を表示
    st.sidebar.markdown("## 利用目的")
    # 「社内文書検索」と「社内問い合わせ」の選択肢を表示
...
def display_initial_ai_message():
    ...
    with st.chat_message("assistant"):
        ...
        # "具体的に入力した方が期待通りの回答を得やすいです。"というメッセージを警告アイコン付きで表示
        st.warning("具体的に入力した方が期待通りの回答を得やすいです。")

        #"【「社内文書検索」を選択した場合】"から"人事部に所属している従業員情報を一覧化して"までをサイドバーに移動
        # 「社内文書検索」の機能説明
                        ...
                        if "sub_message" in message["content"]:
                            ...
                            for sub_choice in message["content"]["sub_choices"]:
                                ...
                                if "page_number" in sub_choice:
                                    st.info(f"{sub_choice['source']}(ページNo. {message['content']['page_number']})", icon=icon)
                                else:
                    ...
                    if "file_info_list" in message["content"]:
                        ...
                        for file_info in message["content"]["file_info_list"]:
                            ...
                            if "page" in file_info:
                                ...
                                # 「ファイルパス」と「ページ番号」を表示
                                file_info = f"{file_info} (ページNo.{page_number})"
                            else:
                                # 「ファイルパス」のみを表示
